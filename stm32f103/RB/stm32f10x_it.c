...
{  
  if (SPI_I2S_GetITStatus(SPI_SLAVE, SPI_I2S_IT_TXE) != RESET)
  {
    /* Send SPI_SLAVE data */
    SPI_I2S_SendData(SPI_SLAVE, SPI_MASTER_Buffer_Tx[TxIdx++]);

    /* Disable SPI_SLAVE TXE interrupt */
    if (TxIdx == BufferSize)
    {
      SPI_I2S_ITConfig(SPI_SLAVE, SPI_I2S_IT_TXE, DISABLE);
    }
  }
  /*SPI_SLAVE READ DATA*/
